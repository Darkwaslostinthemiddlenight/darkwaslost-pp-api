...
def check_cc(cc_details):
    ...
    # Combine response and code if they exist
    response_message = result.get("response", "")
    if "code" in result:
        response_message += f" | code: {result['code']}"
    simplified_response["response"] = response_message
